import React, { useState } from 'react'
import { Link, useParams } from 'react-router-dom'
import { Main, MainForAuthorized } from '../../page/funcs.js'
import './community.css'


export function Community() {
	
const qq = useParams()
	
	
	return (<>
	
<Main>



<div className="col-9 cmnt2z1qwer">


<div className="cbrd1">

<div className="cbrd122" style={{background:'url(/pics/pexels-luis-del-río-15286.jpg)'}}>
</div>

</div>


<div className="col-12 brwhite">

<div className="c123zdsplt">
<div><img alt="xz"   className="rnmg271_kzr2" src="/pics/channels4_profile.jpg" width="128"/></div>

<div className="ndsg3 cqwe1z214_c1">
<div className="fntq_22">Warm Autumn</div>
<div>@warmautumn7100</div>
<div>2.58K subscribers</div>
</div>


<div className="flwqe_12"><div className="flwqe_1">Follow</div></div>

</div>


<div className="c123zdspltzc123">
<div><Link to="/">Home</Link></div>
<div><Link to="/">About</Link></div>
<div><Link to="/">Members</Link></div>
<div><Link to="/">Videos</Link></div>
<div><Link to="/">Posts</Link></div>
<div><Link to="/">More</Link></div>
</div>


</div>





<div className="c123czt13afqf">

<div className="c13zzzz_all_items">

<div className="cqz active">Photos</div>
<div className="cqz">Videos</div>
<div className="cqz">Music</div>
<div className="cqz">Topics</div>

</div>



<div className="c13zzzz_all_items cqwezzz22itmnlst">

<div><img alt="xz"   src="/pics/CYjP_-UxpPs.jpg"/></div>

<div><img alt="xz"   src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"   src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"   src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"  src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>
<div><img alt="xz"    src="/pics/CYjP_-UxpPs.jpg"/></div>



</div>

<div className="rnmg31">
<Link to="/" className="rnmg32">See all</Link>
</div>

</div>





<div className="col-12">


<div className="snacks6">


<div className="ce12524">


<div>

<div className="left snacks7"><img alt="xz"  className="brdr21" src="/pics/KXTvmL_0KAs.jpg" width="50" height="50"/>	</div>
	
<div className="left snacks8"> 
	<div><Link to="/">Nini koberidze</Link></div>	
	<div><span className="snacks9">6 Jan at 2:00 am</span></div>	
</div>	


<div className="right snacks10"> 
	<div><Link to="/"> <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle><circle cx="5" cy="12" r="1"></circle></svg> </Link></div>		
</div>	




<div className="crlrbth">
			
			<div className="text">
	
				Nice very nice, i like this one !!!</div>
			
			
			
			</div>




<div className="pstfl">
			
			
			<div>
				
				<div className="czinlwqeq">
			    <div className="hrt1"> </div>
				<div className="cz2z_fnt"> 25 </div>
				</div>
				

			</div>
			
		
		
			
			
						<div>
				
				<div className="czinlwqeq">
			    <div className="cmntz_on_cwpad"> </div>
				<div className="cz2z_fnt"> 35 </div>
				</div>
				

			</div>
			
			
				
				
				<div>
				
				<div className="czinlwqeq">
			    <div className="cp_share"> </div>
				<div className="cz2z_fnt"> 35 </div>
				</div>
				

			</div>
			
			

<div className="right c123c1add"><svg stroke="currentColor" fill="#c4c8cd" stroke-width="0" viewBox="0 0 1024 1024" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M396 512a112 112 0 1 0 224 0 112 112 0 1 0-224 0zm546.2-25.8C847.4 286.5 704.1 186 512 186c-192.2 0-335.4 100.5-430.2 300.3a60.3 60.3 0 0 0 0 51.5C176.6 737.5 319.9 838 512 838c192.2 0 335.4-100.5 430.2-300.3 7.7-16.2 7.7-35 0-51.5zM508 688c-97.2 0-176-78.8-176-176s78.8-176 176-176 176 78.8 176 176-78.8 176-176 176z"></path></svg> 35</div>



			
			</div>








</div>
























</div>





</div>

</div>






<div className="col-12">


<div className="snacks6">


<div className="ce12524">


<div>

<div className="left snacks7"><img alt="xz"  className="brdr21" src="/pics/KXTvmL_0KAs.jpg" width="50" height="50"/>	</div>
	
<div className="left snacks8"> 
	<div><Link to="/">Nini koberidze</Link></div>	
	<div><span className="snacks9">6 Jan at 2:00 am</span></div>	
</div>	


<div className="right snacks10"> 
	<div><Link to="/"> <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle><circle cx="5" cy="12" r="1"></circle></svg> </Link></div>		
</div>	




<div className="crlrbth">
			
			<div className="text">
	
				Pretty girl ❤️❤️❤️
				</div>
			
			
			<div className="img_detected_on_pst">
			<img alt="xz"  src="/pics/jJ7g1VXRvqw.jpg"/>
			</div>
			
			
			</div>




<div className="pstfl">
			
			
			<div>
				
				<div className="czinlwqeq">
			    <div className="hrt1"> </div>
				<div className="cz2z_fnt"> 25 </div>
				</div>
				

			</div>
			
		
		
			
			
						<div>
				
				<div className="czinlwqeq">
			    <div className="cmntz_on_cwpad"> </div>
				<div className="cz2z_fnt"> 35 </div>
				</div>
				

			</div>
			
			
				
				
				<div>
				
				<div className="czinlwqeq">
			    <div className="cp_share"> </div>
				<div className="cz2z_fnt"> 35 </div>
				</div>
				

			</div>
			
			

<div className="right c123c1add"><svg stroke="currentColor" fill="#c4c8cd" stroke-width="0" viewBox="0 0 1024 1024" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M396 512a112 112 0 1 0 224 0 112 112 0 1 0-224 0zm546.2-25.8C847.4 286.5 704.1 186 512 186c-192.2 0-335.4 100.5-430.2 300.3a60.3 60.3 0 0 0 0 51.5C176.6 737.5 319.9 838 512 838c192.2 0 335.4-100.5 430.2-300.3 7.7-16.2 7.7-35 0-51.5zM508 688c-97.2 0-176-78.8-176-176s78.8-176 176-176 176 78.8 176 176-78.8 176-176 176z"></path></svg> 35</div>



			
			</div>







</div>
























</div>





</div>

</div>









</div>





	

<div className="col-3">


<div className="rnmg30">



<div className="rprflmn">


<div><Link to="/">News</Link></div>
<div><Link to="/">Reactions</Link></div>
<div><Link to="/">Search</Link></div>
<div><Link to="/">Updates</Link></div>
<div><Link to="/">Comments</Link></div>
<div><Link to="/">Videos</Link></div>
<div><Link to="/">Photos</Link></div>
<div><Link to="/">Recommended</Link></div>
<div><Link to="/">Settings</Link></div>
<div><Link to="/">My page</Link></div>
<div><Link to="/">Pages</Link></div>

</div>

<div className="rnmg31">
	<Link to="/" className="rnmg32">See all</Link>
</div>


</div>












<div className="c123czt13afqf">


<div><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M2 3.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm.646 2.146a.5.5 0 0 1 .708 0l2 2a.5.5 0 0 1 0 .708l-2 2a.5.5 0 0 1-.708-.708L4.293 8 2.646 6.354a.5.5 0 0 1 0-.708zM7 6.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 3a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5zm-5 3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"></path></svg>  

 <span className="txtbrk">
	 Description of qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq
</span>
</div>

</div>



<div className="rnmg30" style={{marginTop:'14px'}}>

<div className="t21">Members <Link to="/"> <b>5000</b> </Link></div>


<div className="cqwezzz22itmnlst">

</div>

<div className="prfrmn">


<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>

<div>
<img alt="xz"  src="/pics/KXTvmL_0KAs.jpg" width="80" height="80"/>	
</div>



</div>


<div className="rnmg31">
<Link to="/" className="rnmg32">See all</Link>
</div>


</div>




</div>	


</Main>	
	
	
	</>)
}

export default function Communities() {
	
	
const [List,setList] = useState([{
	name:'battle thru heavens',
	date: '20/12/2023',
	followers: '1252',
	pic: '/pics/1252.jpg',
	sta: 'following',
	desc: 'Creative work'
},
{
	name:'Against the sky supreme',
	date: '20/12/2022',
	followers: '15',
	pic: '/pics/117031.jpg',
	sta: 'following',
	desc: 'Creative work'
}, 
{
	name: 'Lovely girls',
	date: '20/12/2022',
	followers: '15',
	pic: '/pics/d281cf9550d6abc5e3824f6b55e57983.jpg',
	sta: 'unfollow',
	desc: 'Creative work'
},
{
	name: 'Anime lovers',
	date: '20/12/2022',
	followers: '15',
	pic: '/pics/0oRxrSt0fdw.jpg',
	sta: 'unfollow',
	desc: 'Creative work'
}
]);	
	

return (<>

<MainForAuthorized>


<div className="col-9 cmnt2z1qwer">




<div className="CQWEz_22z">

<div className="Head_Rec_Vis">

<div className="GRFvSTTl"> Recently you visited</div>

<div className="GRFvSTTl right"> <Link to="/">Clear</Link></div>


</div>



<div className="cmn_rq1131">

{/* */}



{List.map((val,index)=> (
<div className="s3" key={index}>
        <div className="flc_xx12_zdre2z">
            <div><img alt="xz"  className="pw_pho231" src="/pics/0ittuJYYBho.jpg"/> </div>	
            <div><a href="/">Myanime.Live</a> </div>	
      </div>
   
</div>
))}
    


{/* */}

</div>



</div>










{/*

   <div className="cz_mnt">
      
      <div>
         <img alt="xz"  src="/pics/7ehztC44L2k.jpg" className="pwr_phro231" width="50" height="50"/>
      </div>	

      <div style={{display:'table'}}>
         <div>qweqweqweqwe</div>
         <div>qweqweqweqwe</div>
      </div>	


      <div>
         <div style={{float:'right'}}>qweqweqweqwe</div>
      </div>
      
      
   </div>

*/}


<div className="cmnt_1521z">



<div className="cdinlb">

<div className="PPzLeft">
<div className="crqwz active">All communities <b>(1)</b></div>
</div>

<div className="PPzLeft">
<div className="crqwz">Managed communities</div>
</div>

</div>

<div className="cqwezdsplbl">


<input type="text" className="ww_f1z" placeholder="Searching by name"/> 

<button className="cqwezz_2"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"></path></svg></button>


</div>



<div className="cmnt_1521zz">
	
	Recommended
</div>








{List.map((val,index)=> (
   
<div className="c_zlfqffl1" key={index}>



         <div>

            <div className="c_zlrqzfqf4fl1">
                     <div>
                        <img alt="xz"  src={val.pic} className="pwr_phro431" width="80" height="80"/>
                     </div>	

                     <div className="pd2">
                        <div><Link to="/" className="v_col31">{val.name}</Link></div>
                        <div className="aa_whl123z">{val.desc}</div>
                        <div><Link to="/" className="aa_whl123z">{val.followers} members</Link></div>
                     </div>
            </div>
               
         
         </div>
            
    <div>  
      <div>
         <div href="" className="flw">{val.sta}</div>
      </div>
    </div>  
      

	
	
</div>


))}  











</div>




</div>


<div className="col-3">


<div className="rprflmn">

<div><Link to="/">News</Link></div>
<div><Link to="/">Reactions</Link></div>
<div><Link to="/">Search</Link></div>
<div><Link to="/">Updates</Link></div>
<div><Link to="/">Comments</Link></div>
<div><Link to="/">Videos</Link></div>
<div><Link to="/">Photos</Link></div>
<div><Link to="/">Recommended</Link></div>
<div><Link to="/">Settings</Link></div>
<div><Link to="/">My page</Link></div>
<div><Link to="/">Pages</Link></div>

</div>

</div>


</MainForAuthorized>



 </>);

}



